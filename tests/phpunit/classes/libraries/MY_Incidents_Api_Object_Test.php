...
class Incidents_Api_Object_Test extends PHPUnit_Framework_TestCase {
	...
	public function testGetIncidentsByLatLon()
	{
		...
		$result = $db->query('SELECT latitude FROM '.$this->table_prefix.'location')->as_array();
		$lat = (count($result) > 0) ? $result[array_rand($result)]->latitude : count($result);
		...
		$this->assertEquals("0", $contents->error->code, sprintf("No records found", $lat));
