...
class reports_Core {
	 ...
	 */
	public static function fetch_incidents($paginate = FALSE, $items_per_page = 0, $cat_id = -1) // Added $cat_id parameter to account for multiple categories
	{
		// Reset the paramters
		self::$params = array();
		...
		if (isset($url_data['c']) AND is_array($url_data['c']))
		{
			...
			{
			    //--------Use Category that is being passed through (TB)-------------------
			    if ($cat_id == -1)
			    {
