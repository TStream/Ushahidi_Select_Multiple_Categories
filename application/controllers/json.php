...
class Json_Controller extends Template_Controller {
	...
	protected function geojson($type)
	{
		...
		// Category ID
		$category_ids = (isset($_GET['c']) AND intval($_GET['c']) > 0) ? $_GET['c'] : 0;
        $array_category_id = preg_split("/,/",$category_ids); // put all the category ids into an array (TB)
        
        foreach ($array_category_id as $category_id) // go through each category id (TB)
        {
            $markers = FALSE;
		    ...
		    if (! $markers)
		    {
			    $markers = (isset($_GET['page']) AND intval($_GET['page']) > 0)
			        ? reports::fetch_incidents(TRUE,0,$category_id)  //added extra parameter to count for mulitiple ids (TB)
			        : reports::fetch_incidents(FALSE,0,$category_id);
		    }
		    ...
		    $feature_array = array_merge($feature_array,$json_features);//array of all the features of multiple categories	

		}
		$this->render_geojson($feature_array);
	}
	...
	protected function clusters_geojson($incidents, $category_id, $color, $icon)
	{
		...
		foreach ($clusters as $cluster)
		{
			...
			$json_item['properties'] = array(
				...
				'link' => $link,
				'category' => array($category_id),
				'color' => $color,//($cluster_count >= 2 ? ($cluster_count >= 4 ? ($cluster_count >= 10 ? "FF0000" : "FF8000") : "FBF80C") : $color),
				'icon' => $icon,
				'thumb' => '',
				'timestamp' => 0,
				'count' => $cluster_count,
