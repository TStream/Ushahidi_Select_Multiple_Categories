...
// To hold multiple categories (added by T.Bach)
...
jQuery(function() {
	...
	// Category Switch Action (Redone by T.Bach (TB) to add the ability to select multple categories)
	$("ul#category_switch li > a").click(function(e) {
		...
		// if already active than decativate
		if ($("#cat_" + categoryId).hasClass("active"))
		{
		   var index = idArray.indexOf(categoryId);
		   if (index > -1) // remove category from idArray
		   {
		       idArray.splice(index, 1);
		   }
		   
		   $("#cat_" + categoryId).removeClass("active");
		   $("#child_" + categoryId).hide();
		   
		   // checks to see if no other children are active and if no other children are active hide children
		...
		else
		{
		    ...
		    try
		    {		    
		        var child = document.getElementById(cid).firstChild.childNodes;
		    }
		    catch(err)
		    {
		        var child = 0;
            ...
            if (child != 0)
            {
                for (var i = 0; i < child.length ; i++){
                    var childId = child[i].firstChild.id.substring(4);
		    ...
		    var parentId = document.getElementById(id).parentNode.parentNode.parentNode.id.substring(6);
		    
		    //deactivates parent
		    if (parentId > 0){	        
		        if ($("#cat_" + parentId).hasClass("active"))
		        {
		            var index = idArray.indexOf(parentId);
		            if (index > -1) // remove category from idArray
		            {
		                idArray.splice(index, 1);
		            }
